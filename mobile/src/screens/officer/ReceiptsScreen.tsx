...
export default function ReceiptsScreen() {
  ...
  const submit = async () => {
    if (!stallId) { setFormError("Choose which stall this payment is for."); return; }
    if (!file) { setFormError("Attach the receipt — photo or file."); return; }
    setFormError("");
    setSubmitting(true);
    try {
      await createReceipt({
            ...
            {receipts.map((r) => {
              ...
              return (
                <Card key={r.id} className="p-4">
                  ...
                  {r.notes ? (
                    <Text className="mt-3 text-xs leading-5 text-gray-600">{r.notes}</Text>
                  ) : null}
      ...
      <Modal visible={formOpen} animationType="slide" onRequestClose={() => setFormOpen(false)}>
        <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
          <View className="flex-row items-center border-b border-gray-200 bg-white px-4 py-3">
            <Pressable onPress={() => { setFormOpen(false); reset(); }} hitSlop={12} className="pr-3">
            ...
            <Card className="mt-4 p-4">
              ...
              {file ? (
                <View className="flex-row items-center rounded-xl border border-gray-200 bg-gray-50 px-3 py-3">
                  ...
                  <Text className="ml-2 flex-1 text-xs text-gray-700" numberOfLines={1}>{file.name}</Text>
