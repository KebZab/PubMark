...
export default function ApplicationFormScreen({ route, navigation }: any) {
  const { stallId, stallName } = route.params as { stallId: string; stallName: string };
  ...
  const [permit, setPermit] = useState<PickedFile | null>(null);
  const [additional, setAdditional] = useState<PickedFile | null>(null);
  ...
  const takePhoto = async (setter: (f: PickedFile) => void) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("Application submitted", `Your application for ${stallName} is now awaiting admin review.`, [
        ...
        <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
              ...
              <View className="flex-row flex-wrap gap-2">
                {BUSINESS_TYPES.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <Text className={`text-xs font-semibold ${active ? "text-white" : "text-gray-600"}`}>{t}</Text>
                    </Pressable>
                  );
                })}
              </View>
          ...
          <Card className="mt-4 p-4">
            ...
            <TextInput
              ...
              placeholderTextColor="#9ca3af"
              value={startDate}
              onChangeText={(v) => { setStartDate(v); setError(""); }}
              editable={!submitting}
              autoCapitalize="none"
            />
                ...
                {TERM_OPTIONS.map((opt) => {
                  ...
                  return (
                    <Pressable
                      ...
                      className={`rounded-full px-3 py-2 ${active ? "bg-primary" : "bg-gray-100"}`}
                    >
                      <Text className={`text-xs font-semibold ${active ? "text-white" : "text-gray-600"}`}>{opt.label}</Text>
                    </Pressable>
                  );
                })}
              </View>
            ...
            <View className="mt-4 rounded-xl bg-primary-surface px-3 py-2.5">
              <Text className="text-xs text-primary-darker">
                {contractEnd ? `Contract ends ${contractEnd}` : "Enter a valid start date to see the end date."}
              </Text>
            </View>
          </Card>
