...
import type { TransferRequest } from "../../services/types";
...
export default function TransfersScreen() {
  ...
  const respond = useCallback(
    async (transfer: TransferRequest, status: "accepted" | "declined") => {
  ...
  const confirmAccept = (transfer: TransferRequest) => {
    Alert.alert(
      "Accept this stall?",
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
      <ScreenHeader
        title="Transfers"
        subtitle={incoming.length > 0 ? `${incoming.length} offer(s) awaiting your decision` : "Stall handovers"}
      />
      ...
      >
        {loading ? (
          ...
          <View className="px-4 pt-4">
                    ...
                    <Card key={t.id} className="overflow-hidden border-primary/40">
                      ...
                      <View className="px-4 py-3">
                        <Text className="text-xs leading-5 text-gray-600">
                          {t.fromUserName} wants to hand {t.stallName} over to you. Accepting releases
                          their claim; you then apply for it to start your own contract.
                        </Text>
                      </View>
            ...
            {history.length > 0 ? (
              <>
                <Text className={`mb-3 text-sm font-semibold text-gray-800 ${incoming.length > 0 ? "mt-7" : ""}`}>
                  History
                </Text>
                <View className="gap-3">
                  {history.map((t) => {
...
function TransferStatusPill({ status }: { status: TransferRequest["status"] }) {
  const style = {
    pending: { bg: "bg-amber-100", text: "text-amber-700", label: "Pending", icon: "time-outline" as const },
    accepted: { bg: "bg-emerald-100", text: "text-emerald-700", label: "Accepted", icon: "checkmark-circle-outline" as const },
    declined: { bg: "bg-gray-200", text: "text-gray-600", label: "Declined", icon: "close-circle-outline" as const },
  }[status];

  return (
    <View className={`flex-row items-center gap-1 self-start rounded-full px-2.5 py-1 ${style.bg}`}>
      <Ionicons name={style.icon} size={11} color={status === "accepted" ? "#047857" : status === "pending" ? "#b45309" : "#4b5563"} />
