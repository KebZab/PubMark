...
export default function MapScreen() {
  ...
  const styleInputs = useMemo<Record<string, StallStyleInput>>(() => {
    ...
    for (const stall of allStalls) {
      ...
      map[stall.id] = {
        userAppStatus: mine ? (mine.status as "pending" | "approved") : null,
        ...
        {FLOORS.map((f) => {
          ...
          return (
            ...
            >
              ...
              <Text className={`text-[10px] ${active ? "text-primary-light" : "text-gray-400"}`}>{counts[f]}</Text>
            </Pressable>
          );
        })}
...
function Note({ tone, text }: { tone: "teal" | "amber" | "indigo" | "gray"; text: string }) {
  const styles = {
