...
function stallStyle(userApp: StoredApplication | null, globalOccupied: boolean, isSelected: boolean) {
  ...
  if (userApp?.status === "pending") {
    return { color: isSelected ? "#d97706" : "#f59e0b", fillColor: isSelected ? "#d97706" : "#f59e0b", weight: isSelected ? 3 : 2, fillOpacity: isSelected ? 0.4 : 0.25 };
  }
  if (globalOccupied) {
    return { color: isSelected ? "#6b7280" : "#9ca3af", fillColor: isSelected ? "#6b7280" : "#9ca3af", weight: isSelected ? 3 : 2, fillOpacity: isSelected ? 0.4 : 0.25 };
  }
...
function DrawnStallsLayer({
  ...
  useEffect(() => {
    ...
    stalls.forEach((stall) => {
      ...
      layer.bindTooltip(stallTooltipLabel(stall, userApp, globalOccupied), { direction: "top", opacity: 0.95 });
...
export function UserMapDashboard() {
  ...
  const [reportCategory, setReportCategory] = useState<ViolationCategory>("Other");
  ...
  const vacant = floorStalls.filter(
    (s) => !allApplications.some((a) => a.stallId === s.id && a.status === "approved") &&
           !getActiveApp(s.id, userApplications)
  ).length;
  const pendingCount = floorStalls.filter((s) => getActiveApp(s.id, userApplications)?.status === "pending").length;
    ...
    <div className="size-full relative overflow-hidden bg-gray-200 max-w-md mx-auto">
        ...
        <div className="px-3 flex gap-2 pointer-events-auto flex-wrap items-center">
          ...
          {searchQuery && (
            <div className="bg-[#14B8A6]/90 backdrop-blur-md rounded-xl px-3 py-1.5 shadow-md flex items-center gap-1.5">
              <span className="text-xs font-semibold text-white">{filteredStalls.length} found</span>
            </div>
          )}
        </div>
      </div>
      ...
      {storedStalls.length === 0 && (
        <div className="absolute inset-0 z-[500] flex items-center justify-center pointer-events-none">
          <div className="bg-white/90 backdrop-blur-md rounded-2xl px-6 py-5 shadow-xl border border-gray-100 text-center mx-6">
            ...
            <p className="text-xs text-gray-400 mt-1">The admin hasn't drawn any stalls on the map.</p>
          </div>
        </div>
      )}
      ...
      {storedStalls.length > 0 && floorStalls.length === 0 && !searchQuery && (
        <div className="absolute inset-0 z-[500] flex items-center justify-center pointer-events-none">
          <div className="bg-white/90 backdrop-blur-md rounded-2xl px-6 py-5 shadow-xl border border-gray-100 text-center mx-6">
            <Store className="w-8 h-8 text-gray-300 mx-auto mb-2" />
            <p className="text-sm font-semibold text-gray-600">No stalls on {activeFloor === "1" ? "1st" : "2nd"} Floor</p>
      ...
      {selected && (() => {
        ...
        const globalApp = allApplications.find((a) => a.stallId === selected.id && a.status === "approved") ?? null;
        ...
        const badge = isMyApproved
          ? <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700 flex items-center gap-1"><CheckCircle className="w-3 h-3" />Your Stall</span>
          : isMyPending
          ? <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 flex items-center gap-1"><Clock className="w-3 h-3" />Pending</span>
          : isOtherOccupied
          ? <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">Occupied</span>
          : <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-teal-100 text-[#0d9488]">Available</span>;
        ...
        return (
          <div className="absolute z-[1000] left-3 right-3 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden" style={{ bottom: "16px" }}>
            <div className={`px-5 py-4 flex items-start gap-4 ${headerBg}`}>
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${iconBg}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  Section {selected.section}{selected.floor_area ? ` · ${selected.floor_area}` : ""} · {selected.floor === "1" ? "1st Floor" : "2nd Floor"}
                </p>
              </div>
            ...
            <div className="px-5 py-3">
              {isMyApproved && selectedUserApp ? (
                <div className="flex items-center gap-3 py-1">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{selectedUserApp.businessName}</p>
                    <p className="text-xs text-gray-500">{selectedUserApp.applicantName} · {selectedUserApp.businessType}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 py-1">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{selectedUserApp.businessName}</p>
                    <p className="text-xs text-gray-500">{selectedUserApp.applicantName} · Application under review</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 py-1">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{globalApp.businessName}</p>
                    <p className="text-xs text-gray-500">{globalApp.applicantName} · {globalApp.businessType}</p>
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-3 py-1">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800">{selected.business_type || "General"}</p>
        ...
        <div className="absolute inset-0 z-[2000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-red-600 to-orange-500 px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={() => { setShowReportModal(false); setReportDescription(""); setReportCategory("Other"); }} className="w-7 h-7 bg-white/20 rounded-lg flex items-center justify-center">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Violation Category</label>
                <select
                  value={reportCategory}
                  onChange={(e) => setReportCategory(e.target.value as ViolationCategory)}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => { setShowReportModal(false); setReportDescription(""); setReportCategory("Other"); }}
