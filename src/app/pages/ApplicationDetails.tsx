...
export function ApplicationDetails() {
  ...
  useEffect(() => {
    ...
    if (app && app.status !== "pending" && id) {
      try {
        const seen: string[] = JSON.parse(localStorage.getItem("pubmark_seen_decisions") ?? "[]");
        if (!seen.includes(id)) {
          seen.push(id);
        ...
        <div className="flex items-center gap-3 px-4 pt-10 pb-4">
          ...
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-gray-400 truncate">{app.stallName} · {app.businessName}</p>
            </div>
          </div>
        ...
        <div className="p-4 space-y-4 pb-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <InfoBlock
                label="Start Date"
                value={new Date(app.contractStart).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
              />
              <div className="col-span-2">
                <InfoBlock
                  label="End Date"
                  value={new Date(app.contractEnd).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                />
              </div>
            </div>
          </div>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 pb-4">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {dateApplied.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
                  </p>
                </div>
              </div>
              <div className="flex gap-3">
          ...
          {/* Documents */}
          <div className={`bg-white rounded-2xl shadow-sm border p-4 ${app.status === "approved" && !app.permitFileName ? "border-amber-300" : "border-gray-200"}`}>
            <div className="flex items-center gap-2 mb-3">
            ...
            <div className="space-y-2">
              {app.permitFileName ? (
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-gray-900 truncate">{app.permitFileName}</p>
                    <p className="text-[10px] text-gray-400">Business Permit{app.permitFileSize ? ` · ${app.permitFileSize}` : ""}</p>
                  </div>
                  <span className="text-[10px] px-2 py-0.5 bg-teal-100 text-teal-700 rounded-full font-medium flex-shrink-0">Uploaded</span>
                </div>
              ) : (
                ...
                <div>
                  ...
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      ...
                      <Upload className={`w-5 h-5 ${app.status === "approved" ? "text-amber-600" : "text-gray-400"}`} />
                    </div>
                    <div className="flex-1">
                      <p className={`text-sm font-semibold ${app.status === "approved" ? "text-amber-900" : "text-gray-700"}`}>
                        {uploading ? "Uploading…" : "Upload Business Permit"}
                      </p>
                      <p className={`text-xs mt-0.5 ${app.status === "approved" ? "text-amber-700" : "text-gray-400"}`}>
                        {app.status === "approved"
                          ? "Required for your approved stall — tap to upload"
                          : "PDF, DOC, JPG, PNG · tap to upload"}
                      </p>
                    </div>
                    <ChevronRight className={`w-4 h-4 flex-shrink-0 ${app.status === "approved" ? "text-amber-500" : "text-gray-300"}`} />
                  </label>
                </div>
              )}
              ...
              {app.additionalFileName ? (
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-gray-900 truncate">{app.additionalFileName}</p>
                    <p className="text-[10px] text-gray-400">Additional{app.additionalFileSize ? ` · ${app.additionalFileSize}` : ""}</p>
                  </div>
                  <span className="text-[10px] px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full font-medium flex-shrink-0">Optional</span>
                </div>
              ) : null}
            </div>
          </div>
            ...
            {permitMeta.visibleRemarks ? (
              ...
              <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-100">
                <p className="text-xs text-gray-400 italic">
                  {app.status === "pending" ? "No remarks yet — under review." : "No remarks provided."}
                </p>
              </div>
            )}
          </div>
          ...
          {app.status === "approved" && (
            hasPendingTransfer ? (
              <div className="w-full flex items-center gap-3 bg-purple-50 border border-purple-200 rounded-2xl p-4">
                ...
                <p className="flex-1 text-sm font-medium text-purple-700 text-left">Transfer offer sent — awaiting response</p>
              </div>
            ) : (
              <button
              ...
              >
                ...
                <p className="flex-1 text-sm font-medium text-gray-800 text-left">Transfer Ownership</p>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-sm overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-4">
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 leading-relaxed">
                The new owner will fill out an application form. Your approval will be voided once they confirm the transfer.
              </div>
              <div>
                ...
                {transferError && (
                  <p className="mt-1.5 text-xs text-red-600">{transferError}</p>
                )}
              </div>
            </div>
