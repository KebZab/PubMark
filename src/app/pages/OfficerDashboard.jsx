...
import { getCheckRequests, updateCheckRequestStatus } from "../components/checkRequestsStore";
import {
...
async function loadOfficerCheckRequests(officerId, officerName) {
  ...
  return [
    ...assignedRequests.map((request) => ({ ...request, requestSource: "check" })),
    ...legacyAdminRequests,
...
export function OfficerDashboard() {
  ...
  const resolveEvidenceRef = useRef(null);
  ...
  const completeFileRef = useRef(null);
  ...
  const receiptActiveApp = receiptForm.stallId
    ? (applications
        .filter((a) => a.stallId === receiptForm.stallId && a.status === "approved")
  ...
  async function handleSubmitViolation() {
    ...
    try {
      ...
      showToast(`Failed to record request: ${error.message}`, "error");
    }
  }
  ...
  async function handleResolve() {
    ...
    try {
      ...
      const actionLabel =
        resolveModal.action === "ongoing" ? "marked as ongoing" : resolveModal.action;
            ...
            <div className="mx-4 mt-4 bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl p-5 shadow-lg shadow-amber-500/20 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-amber-100 text-xs mt-1">
                  Monitor violations and enforce market regulations.
                </p>
              </div>
            </div>
                  ...
                  {CATEGORIES.map((cat) => {
                    ...
                    return (
                      <div key={cat} className="flex items-center gap-3">
                        <span
                          className={`text-[11px] font-medium px-2.5 py-0.5 rounded-full w-44 truncate ${CATEGORY_COLORS[cat]}`}
                        >
                          {cat}
                        </span>
                        <div className="flex-1 bg-gray-100 rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (count / violations.length) * 100 * 2)}%`,
                            }}
                          />
                        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-[10px] font-medium px-2 py-0.5 rounded-full capitalize ${PRIORITY_COLOR[r.priority] ?? PRIORITY_COLOR.normal}`}
                        >
                          {r.priority} priority
                        </span>
              ...
              <div className="px-4 space-y-2.5">
                {checkRequests.length === 0 ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-xs text-gray-500">
                      Admin-assigned requests will appear here
                    </p>
                  </div>
                ) : (
                  checkRequests.map((r) => {
                    ...
                    .map((v) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div
                              className={`mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${cfg.color}`}
                            >
        ...
        {activeTab === "receipts" && (
          <div className="p-4 space-y-4 pb-24">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-3">
              ...
              {receiptForm.stallId && (
                <div
                  className={`rounded-xl p-3 text-xs ${receiptActiveApp ? "bg-teal-50 border border-teal-200 text-teal-700" : "bg-red-50 border border-red-200 text-red-700"}`}
                >
                  {receiptActiveApp
                    ? `Vendor: ${receiptActiveApp.applicantName}`
                    : "No active vendor for this stall."}
                </div>
              )}
              ...
              <button
                ...
                disabled={
                  !receiptForm.stallId || !receiptFile || !receiptActiveApp || submittingReceipt
                }
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setReqViolationForm((p) => ({ ...p, category: e.target.value }))}
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setReqViolationForm((p) => ({ ...p, description: e.target.value }))
                  }
                  ...
                  <div className="mt-2 space-y-1.5">
                    {reqViolEvidence.map((f, i) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => setReqViolEvidence((p) => p.filter((_, j) => j !== i))}
                        >
        ...
        <div className="absolute inset-0 z-[2000] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 max-h-[85vh] overflow-y-auto">
            ...
            <div className="mb-4">
              <label className="block text-xs font-medium text-gray-700 mb-1.5">
                Proof Image{" "}
                {resolveModal.action !== "ongoing" && <span className="text-red-500">*</span>}
              </label>
              <input
                type="file"
                ...
                <div className="mt-2 space-y-1.5">
                  {resolveEvidence.map((f, i) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-700 flex-1 truncate">{f.name}</span>
                      <button
                        onClick={() => setResolveEvidence((p) => p.filter((_, j) => j !== i))}
                      >
                        <X className="w-3.5 h-3.5 text-gray-400 hover:text-red-500" />
                      </button>
                    </div>
      ...
      {completeModal && (
        <div className="absolute inset-0 z-[2000] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 max-h-[85vh] overflow-y-auto">
            ...
            <p className="text-xs text-gray-500 mb-4">
              Provide a summary and attach photo evidence before completing this request.
            </p>
                  ...
                  <div className="mt-2 space-y-1">
                    {completeFiles.map((f, i) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setCompleteFiles((prev) => prev.filter((_, idx) => idx !== i))
                          }
                        >
              ...
              <button
                onClick={async () => {
                  if (!completeSummary.trim()) {
                    showToast("Please provide a summary report.", "error");
                    return;
                  }
                  if (completeFiles.length === 0) {
                    showToast("Please attach at least one photo.", "error");
                    return;
                  }
                  try {
                    const activeRequest = checkRequests.find(
                      (request) => request.id === completeModal,
                    );
                    if (!activeRequest) {
                      showToast("Request not found.", "error");
                      return;
                    }
                    await updateCheckRequestStatus(
                      completeModal,
                      "completed",
                      "",
