  ...
  useEffect(() => {
    ...
    stalls.forEach((stall) => {
      ...
      const layer = L.geoJSON(
        ...
        { style: { color, fillColor: color, weight: isSelected ? 3 : 2, opacity: 0.95, fillOpacity } }
      );
      layer.bindTooltip(
  ...
  return (
    <div className="size-full relative overflow-hidden bg-gray-200 max-w-md mx-auto">
        ...
        <div className="pointer-events-auto bg-white/96 backdrop-blur-md border-b border-gray-200/80 shadow-sm px-4 py-2.5 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <span className="text-[10px] text-gray-400 block leading-tight">Browse Available Stalls</span>
            </div>
          </div>
      ...
      {stalls.length > 0 && floorStalls.length === 0 && (
        <div className="absolute inset-0 z-[500] flex items-center justify-center pointer-events-none" style={{ top: "100px" }}>
          <div className="bg-white/90 backdrop-blur-md rounded-2xl px-6 py-5 shadow-xl border border-gray-100 text-center mx-6">
            <Store className="w-8 h-8 text-gray-300 mx-auto mb-2" />
            <p className="text-sm font-semibold text-gray-600">No stalls on {activeFloor === "1" ? "1st" : "2nd"} Floor</p>
            <p className="text-xs text-gray-400 mt-1">Try switching floors to see available stalls.</p>
          </div>
        </div>
      )}
      ...
      {!multiSelectMode && selected && (
        <div className="absolute z-[1000] left-3 right-3 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden" style={{ bottom: "16px" }}>
          ...
          <div className={`px-5 py-4 flex items-start gap-4 ${selectedOccupied ? "bg-gradient-to-r from-gray-50 to-slate-50" : "bg-gradient-to-r from-teal-50 to-cyan-50"}`}>
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${selectedOccupied ? "bg-gray-400" : "bg-gradient-to-br from-[#14B8A6] to-[#0d9488]"}`}>
              <Store className="w-5 h-5 text-white" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="font-bold text-gray-900 text-base">{selected.stall_name}</p>
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${selectedOccupied ? "bg-gray-100 text-gray-600" : "bg-teal-100 text-teal-700"}`}>
            ...
            {selectedOccupied && approvedApp ? (
              <div className="flex items-center gap-3 py-1">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{approvedApp.applicantName} · {approvedApp.businessType}</p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3 py-1">
