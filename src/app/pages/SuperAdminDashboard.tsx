...
import {
  Users, ShieldCheck, Store, BarChart3, UserPlus, Trash2,
...
type Tab = "overview" | "users" | "map" | "stalls" | "applications" | "violations" | "check-requests";
...
const EMPTY_FORM: UserFormData = {
  ...
  password: "", role: "vendor", department: "",
};

type SortField = "name" | "email" | "role" | "phone" | "createdAt";
...
export function SuperAdminDashboard() {
  ...
  const [selectedStallForRequest, setSelectedStallForRequest] = useState<string | null>(null);
  ...
  function SortIcon({ field }: { field: SortField }) {
    ...
    return sortAsc ? <ChevronUp className="w-3 h-3 text-purple-600" /> : <ChevronDown className="w-3 h-3 text-purple-600" />;
  }
  ...
  const TABS: { id: Tab; label: string; icon: React.ElementType; badge?: number | string }[] = [
    ...
    { id: "check-requests", label: "Send Request", icon: CheckCircle, badge: openCheckRequestsCount > 0 ? openCheckRequestsCount : undefined },
  ];
        ...
        {tab === "overview" && (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: "Total Users", value: stats.total, icon: Users, color: "bg-purple-50 text-purple-700", badge: "bg-purple-100" },
                { label: "Vendors", value: stats.vendors, icon: Store, color: "bg-teal-50 text-teal-700", badge: "bg-teal-100" },
                { label: "Open Violations", value: stats.openViolations, icon: AlertTriangle, color: "bg-red-50 text-red-700", badge: "bg-red-100" },
                { label: "Pending Applications", value: stats.pendingApps, icon: Activity, color: "bg-amber-50 text-amber-700", badge: "bg-amber-100" },
              ].map((s) => {
                const Icon = s.icon;
                return (
                  <div key={s.label} className="bg-white rounded-2xl border border-gray-200 p-5 flex items-center gap-4 shadow-sm">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${s.badge}`}>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
              ...
              <div className="space-y-3">
                {(["super_admin", "admin", "vendor", "officer"] as UserRole[]).map((r) => {
                  const count = users.filter((u) => u.role === r).length;
                  const pct = users.length ? Math.round((count / users.length) * 100) : 0;
                  return (
                    <div key={r} className="flex items-center gap-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full w-24 text-center ${ROLE_COLORS[r]}`}>
                        {ROLE_LABELS[r]}
                      </span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: "Total Stalls", value: stalls.length, sub: `${stats.occupiedStalls} occupied` },
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <button onClick={() => handleTabChange("applications")} className="text-xs text-purple-600 font-medium hover:underline">
                  View all
                </button>
              </div>
                  ...
                  .map((app) => (
                    <div key={app.id} className="px-5 py-3 flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 truncate">{app.stallName} • {app.businessName}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-400">{formatDate(app.dateApplied)}</span>
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <button onClick={() => handleTabChange("users")} className="text-xs text-purple-600 font-medium hover:underline">
                  View all
                </button>
              </div>
              <div className="divide-y divide-gray-100">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Draw and manage stalls on the map</p>
                  </div>
                  <button
                        ...
                        <td className="px-4 py-3">
                          {editingStall === s.id ? (
                            <select
                              ...
                              onChange={(e) => setEditStallData((prev) => ({ ...prev, status: e.target.value }))}
                            ...
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                              s.status === "vacant" ? "bg-green-100 text-green-700" :
                        ...
                        <td className="px-4 py-3">
                          {editingStall === s.id ? (
                            <input
                              ...
                              onChange={(e) => setEditStallData((prev) => ({ ...prev, business_type: e.target.value }))}
                            ...
                            {editingStall === s.id ? (
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  setEditStallData({ status: s.status, business_type: s.business_type });
                                }}
            ...
            {selectedApplication && (
              <div className="w-96 flex-shrink-0 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-y-auto flex flex-col">
                ...
                <div className="px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-purple-50 to-white flex items-center justify-between flex-shrink-0">
                  ...
                  <button onClick={() => setSelectedApplication(null)} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center">
                ...
                <div className="p-5 space-y-5 flex-1 overflow-y-auto">
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3.5 space-y-2">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-800">{selectedApplication.applicantName}</p>
                          <p className="text-xs text-gray-500">{selectedApplication.applicantEmail}</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-2">
                        <MapPin className="w-3.5 h-3.5 text-gray-400 flex-shrink-0 mt-0.5" />
                        <p className="text-xs text-gray-600">{selectedApplication.applicantAddress}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Business info */}
                  <div>
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Business Details</p>
                    <div className="bg-gray-50 rounded-xl p-3.5 space-y-2">
                      <div>
                        <p className="text-[10px] text-gray-500">Business Name</p>
                        <p className="text-sm font-medium text-gray-800">{selectedApplication.businessName}</p>
                      </div>
                      <div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3.5 space-y-2">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <p className="text-xs font-medium text-gray-800">{formatDate(selectedApplication.contractStart)}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-gray-500">End Date</p>
                          <p className="text-xs font-medium text-gray-800">{formatDate(selectedApplication.contractEnd)}</p>
                        </div>
                      </div>
                      <div>
                        <p className="text-[10px] text-gray-500">Contract Term</p>
                        <p className="text-xs font-medium text-gray-800">{selectedApplication.contractTermMonths} months</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Stall details */}
                  <div>
                    <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Stall Information</p>
                    <div className="bg-gray-50 rounded-xl p-3.5 space-y-2">
                      <div>
                        <p className="text-[10px] text-gray-500">Stall</p>
                        <p className="text-sm font-medium text-gray-800">{selectedApplication.stallName}</p>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <p className="text-[10px] text-gray-500">Section</p>
                          <p className="text-xs text-gray-700">{selectedApplication.stallSection || "—"}</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-gray-500">Floor Area</p>
                          <p className="text-xs text-gray-700">{selectedApplication.floorArea || "—"}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedApplication.notes && (
                    <div>
                      ...
                      <div className="bg-gray-50 rounded-xl p-3.5">
                        <p className="text-xs text-gray-700 leading-relaxed">{selectedApplication.notes}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedApplication.adminRemarks && (
                    <div>
                      ...
                      <div className="bg-amber-50 rounded-xl p-3.5 border border-amber-200">
                        <p className="text-xs text-amber-900 leading-relaxed">{selectedApplication.adminRemarks}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {filtered.map((v) => {
                    ...
                    return (
                      <div key={v.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        >
                          <div className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${v.status === "open" ? "bg-red-500" : v.status === "resolved" ? "bg-green-500" : "bg-gray-400"}`} />
                          <div className="flex-1 min-w-0">
                              ...
                              {r.completionFiles.length === 0 ? (
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  {r.completionFiles.map((f, idx) => (
                                    <div key={idx} className={`rounded-xl overflow-hidden border ${f.type === "image" ? "border-purple-200" : f.type === "video" ? "border-blue-200" : "border-gray-200"}`}>
                                      {f.type === "image" ? (
                                        <div className="bg-gradient-to-br from-purple-50 to-indigo-50 h-20 flex flex-col items-center justify-center gap-1">
                                          <Eye className="w-6 h-6 text-purple-400" />
                                          <span className="text-[9px] font-medium text-purple-600">Photo</span>
                                        </div>
                                      ) : f.type === "video" ? (
                                        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 h-20 flex flex-col items-center justify-center gap-1">
                                          <FileText className="w-6 h-6 text-blue-400" />
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">Request officers to check stalls for potential violations</p>
              </div>
              <button
                            ...
                            <td className="px-4 py-3">
                              {assigningRequest === req.id && req.status !== "completed" ? (
                                <select
                                  ...
                                  onChange={(e) => handleAssignRequestOfficer(req.id, e.target.value)}
                                ...
                                <span className="text-gray-700">{req.assignedOfficerName || "—"}</span>
                              )}
                            </td>
                                      ...
                                      <>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                          {req.completionFiles.map((f, i) => (
                                            <span key={i} className="flex items-center gap-1 px-2 py-1 bg-white border border-purple-200 rounded-lg text-[10px] text-gray-700">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Select Stall</label>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                          s.status === "vacant" ? "bg-green-100 text-green-700" :
                          s.status === "occupied" ? "bg-blue-100 text-blue-700" :
                          "bg-gray-100 text-gray-700"
                        }`}>
                          {s.status}
                        </span>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1.5">Reason for Check</label>
                <textarea
                  value={requestReason}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-purple-600 to-purple-500 px-6 py-4 flex items-center justify-between">
              <h2 className="text-white font-semibold">{editUser ? "Edit User" : "Add New User"}</h2>
              <button onClick={() => setShowForm(false)} className="w-7 h-7 bg-white/20 rounded-lg flex items-center justify-center">
                <X className="w-4 h-4 text-white" />
              </button>
            </div>
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              {[
                ...
                { field: "email", label: "Email Address", placeholder: "user@example.com", type: "email" },
                ...
                { field: "department", label: "Department (optional)", placeholder: "e.g. Market Operations", type: "text" },
              ...
              <div>
                ...
                >
                  {(["vendor", "officer", "admin", "super_admin"] as UserRole[]).map((r) => (
                    <option key={r} value={r}>{ROLE_LABELS[r]}</option>
                  ))}
                </select>
              </div>
      ...
      {terminationActionConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4 ${terminationActionConfirm.action === "approved" ? "bg-green-100" : "bg-gray-100"}`}>
              {terminationActionConfirm.action === "approved"
