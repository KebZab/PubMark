...
// ── Contract Info Modal ───────────────────────────────────────────────────────
function ContractModal({ stall, app, onClose }) {
        ...
        <div className="px-6 py-5 flex items-center justify-between bg-gradient-to-r from-amber-50 to-yellow-50 border-b border-amber-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-gray-900">{stall.stall_name}</p>
              <p className="text-xs text-gray-500">
                {apps.length} applicants · none approved yet
              </p>
            </div>
          </div>
          <button
  ...
  return (
    <div className="size-full bg-gray-50 overflow-y-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              {stats.total} stalls · {stats.occupied} occupied · {stats.vacant} vacant
            </p>
          </div>
        </div>
      </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="px-5 py-3.5 text-right text-xs font-semibold text-gray-500 uppercase">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {filtered.map(({ stall, app, apps }) => {
                    const isOccupied = app?.status === "approved";
                    const isPending = app?.status === "pending";
                    // More than one vendor waiting on a decision — nothing is
                    // approved yet, so every entry in `apps` is still pending.
      ...
      {/* Competing applicants modal — tapping one drills into its full
          detail via the same ContractModal used everywhere else. */}
      {applicantsModal && (
        <ApplicantsModal
