...
import {
  ...
  CheckCircle, MapPin, ChevronRight, Layers,
} from "lucide-react";
        ...
        <div className={`px-6 py-5 flex items-center justify-between ${
          ...
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
              app.status === "approved" ? "bg-gradient-to-br from-indigo-500 to-purple-600" : "bg-gradient-to-br from-amber-400 to-orange-500"
            }`}>
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-88px)] p-6 space-y-5">
          ...
          {app.status === "approved" && (
            <div className={`rounded-xl p-4 border flex items-center gap-3 ${
              isExpired ? "bg-red-50 border-red-200" : isExpiringSoon ? "bg-amber-50 border-amber-200" : "bg-emerald-50 border-emerald-200"
            }`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                ...
                : isExpiringSoon ? <Clock className="w-5 h-5 text-amber-600" />
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-3">
              {[
                ...
                { icon: Building2, label: "Business", value: `${app.businessName} · ${app.businessType}` },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Occupied", value: stats.occupied, color: "text-indigo-600", dot: "bg-indigo-500" },
        ].map(({ label, value, color, dot }) => (
          <div key={label} className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-500">{label}</span>
              {dot ? <div className={`w-3 h-3 rounded-full ${dot}`} /> : <Store className="w-5 h-5 text-gray-400" />}
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 h-11 px-3 bg-gray-50 border border-gray-200 rounded-xl">
            ...
            <select
              value={filterFloor}
              onChange={(e) => setFilterFloor(e.target.value as typeof filterFloor)}
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                            ...
                            : isPending ? "bg-amber-100" : "bg-teal-50"
                          }`}>
