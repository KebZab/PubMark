...
function getRoleColor(role: UserRole): string {
  switch (role) {
    ...
    case "vendor": return "bg-teal-100 text-teal-700";
    case "officer": return "bg-amber-100 text-amber-700";
  }
}

function getRoleIcon(role: UserRole): React.ElementType {
  switch (role) {
    case "super_admin": return ShieldCheck;
    case "admin": return UserCog;
    case "vendor": return Store;
    case "officer": return AlertTriangle;
  }
}
...
export function DashboardLayout({ session, children, title, subtitle, actions }: Props) {
      ...
      <div className="px-4 py-3 border-b border-white/10">
        <div className="flex items-center gap-3 bg-white/10 rounded-xl px-3 py-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className={`inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded-full mt-0.5 ${getRoleColor(role)}`}>
              {getRoleLabel(role)}
            </span>
          </div>
        </div>
