      ...
      const status = occupied
        ...
        : alreadyPicked
          ...
          : isStaged
            ? "Selected"
            : "Available";
      layer.bindTooltip(`<strong>${stall.stall_name}</strong> · ${status}`, { direction: "top", opacity: 0.95 });
...
export function AddStallMapPicker({
  stalls,
  applications,
  alreadyPickedIds,
  onConfirm,
  ...
  function toggleStall(stall: Stall) {
    setStagedIds((prev) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-lg h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-t border-gray-100 flex items-center gap-3 flex-shrink-0">
          ...
          >
            Add {stagedIds.size > 0 ? `${stagedIds.size} ` : ""}Stall{stagedIds.size === 1 ? "" : "s"}
