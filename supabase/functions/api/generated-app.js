import express, { cors } from "./edge-express.ts";
...
if (!(process.env.DATABASE_URL || process.env.SUPABASE_DB_URL)) throw new Error("DATABASE_URL is required. Copy server/.env.example to server/.env and fill in real values.");
...
function apiPublicUrl() {
  return process.env.API_PUBLIC_URL || `${process.env.SUPABASE_URL}/functions/v1/api`;
}
